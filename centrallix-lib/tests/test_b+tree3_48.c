...
test(char** tname)
   	{
    *tname = "b+tree3_48 Test bptFromLookup iterator function with key 0018 going backwards";
    int i;
    ...
    *info6 = 180;
    ...
    pBPIter iterator = bptFromLookup(tree, 1, "0018", 4);

    assert(iterator->Ref == info18);
    assert(strcmp(iterator->Curr->Keys[iterator->Index].Value, "0018") == 0);
    bptNext(iterator, &status);
    assert(iterator->Ref == info17);
