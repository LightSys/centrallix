...
test(char** tname)
	...
	*tname = "qprintf-17 %NSTR insertion in middle without overflow";
	...
	for(i=0;i<iter;i++)
	    ...
	    buf[1] = 0xff;
	    buf[0] = '\0';
	    qpfPrintf(session, buf+4, 36, "Here is the string: %6STR...", "STRING");
	    qpfPrintf(session, buf+4, 36, "Here is the string: %6STR...", "STRING");
	    qpfPrintf(session, buf+4, 36, "Here is the string: %6STR...", "STRING");
	    rval = qpfPrintf(session, buf+4, 36, "Here is the string: %6STR...", "STRING");
	    ...
	    /* UTF-8 */
	    rval = qpfPrintf(NULL, buf+4, 36, "Энд мөр байна %6STR", "끈");
