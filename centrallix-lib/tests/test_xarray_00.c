...
static bool do_tests(void)
	...
	/** Get a random value that is not one of the previous values. **/
	int possible_value_not;
	do possible_value_not = rand();
	while (possible_value_not == value1
	    || possible_value_not == value2
	    || possible_value_not == value3
	    || possible_value_not == value4
	    || possible_value_not == value5
	);
