...
static bool doTest(void)
	...
	success &= ASSERT_EQL(xhInit(&hash, HASH_ROWS, KEY_LEN), 0, "%d");
	success &= ASSERT_EQL(hash.KeyLen, KEY_LEN, "%d");
	...
	success &= ASSERT_EQL(xhLookup(&hash, long_key), data[0], "%p");
	success &= ASSERT_EQL(xhAdd(&hash, long_key, "duplicate"), -1, "%d");
	...
	success &= ASSERT_EQL(xhLookup(&hash, other_key), NULL, "%p");
	success &= ASSERT_EQL(xhRemove(&hash, other_key), -1, "%d");

	/** Removing by an equal copy of a key removes the original entry. **/
	success &= ASSERT_EQL(xhRemove(&hash, long_key), 0, "%d");
	success &= ASSERT_EQL(xhLookup(&hash, keys[0]), NULL, "%p");
	success &= ASSERT_EQL(hash.nItems, KEY_COUNT - 1, "%d");
