...
test(char** tname)
	...
	*tname = "b+tree-21 bpt_i_GetNeighborIndex";
	iter = 8000000;
	...
	for(i=0;i<iter;i++)
	 	{
		tmp = bpt_i_GetNeighborIndex(NULL);
		assert (tmp == -2);
		tmp = bpt_i_GetNeighborIndex(root);
		assert (tmp == -3);
		tmp = bpt_i_GetNeighborIndex(leftMid);
		assert (tmp == -1);
		tmp = bpt_i_GetNeighborIndex(rightR);
		assert (tmp == 0);
		}
