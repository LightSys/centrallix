...
static bool doTest(void)
	...
	for (int i = 0; i < ITEM_COUNT; i++)
	    ...
	    if (!ASSERT_NOT_NULL(value)) return false;
	    *value = rand();
	    success &= ASSERT_EQL(xhAdd(&hash, keys[i], (char*)value), 0, "%d");
	    char* found = xhLookup(&hash, keys[i]);
	    success &= ASSERT_EQL(found, (char*)value, "%p");
	    if (found != NULL) success &= ASSERT_EQL(*(int*)found, *value, "%d");
	    }
	success &= ASSERT_EQL(hash.nItems, ITEM_COUNT, "%d");

	/** An entry may carry NULL data; only its key marks its presence. **/
	success &= ASSERT_EQL(xhAdd(&hash, "null-data", NULL), 0, "%d");
	success &= ASSERT_EQL(hash.nItems, ITEM_COUNT + 1, "%d");
	success &= ASSERT_EQL(xhLookup(&hash, "null-data"), NULL, "%p");

	/** Such an entry is still found by a removal, and by a duplicate add. **/
	success &= ASSERT_EQL(xhAdd(&hash, "null-data", NULL), -1, "%d");
	success &= ASSERT_EQL(xhRemove(&hash, "null-data"), 0, "%d");
	success &= ASSERT_EQL(hash.nItems, ITEM_COUNT, "%d");
	 ...
	 ***/
	success &= ASSERT_EQL(xhAdd(&hash, "null-data", NULL), 0, "%d");
	success &= ASSERT_EQL(xhClear(&hash, test_free, &freed), 0, "%d");
	success &= ASSERT_EQL(freed, (unsigned int)ITEM_COUNT + 1, "%u");
	...
	success &= ASSERT_EQL(xhAdd(&hash, keys[0], "reused"), 0, "%d");
	success &= ASSERT_STR_EQL(xhLookup(&hash, keys[0]), "reused");
	...
	int* value = nmMalloc(DATA_SIZE);
	if (!ASSERT_NOT_NULL(value)) return false;
	success &= ASSERT_EQL(xhAdd(&hash, keys[0], (char*)value), 0, "%d");
	success &= ASSERT_EQL(xhClear(&hash, test_free, NULL), 0, "%d");
	success &= ASSERT_EQL(freed, (unsigned int)ITEM_COUNT + 1, "%u");
	success &= ASSERT_EQL(hash.nItems, 0, "%d");
