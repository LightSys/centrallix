...
static bool doTest(void)
	...
	{
	    XArray xPairs;
	    if (!check(xaInit(&xPairs, 4))) return false;
	    ...
	    success &= EXPECT_PAIR(Pairs[0], 1, 2, 0.8, 1.0);
	    success &= EXPECT_PAIR(Pairs[1], 1, 6, 0.8, 1.0);
	    success &= EXPECT_PAIR(Pairs[2], 2, 6, 0.8, 1.0);
	}
	...
	/** Test sliding search: Large window. **/
	{
	    ...
	    success &= EXPECT_PAIR(Pairs[0], 1, 2, 0.8, 1.0);
	    // success &= EXPECT_PAIR(Pairs[1], 1, 6, 0.8, 1.0); /* Missed. */
	    success &= EXPECT_PAIR(Pairs[1], 2, 6, 0.8, 1.0);
	}
	...
	/** Test sliding search: Small window. **/
	{
	    ...
	    success &= EXPECT_PAIR(Pairs[0], 1, 2, 0.8, 1.0);
	    // success &= EXPECT_PAIR(Pairs[1], 1, 6, 0.8, 1.0); /* Missed. */
	    // success &= EXPECT_PAIR(Pairs[2], 2, 6, 0.8, 1.0); /* Missed. */
	}
