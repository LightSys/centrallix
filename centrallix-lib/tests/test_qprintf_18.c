...
test(char** tname)
	...
	*tname = "qprintf-18 %NSTR insertion in middle with insert overflow";
	...
	for(i=0;i<iter;i++)
	    ...
	    buf[1] = 0xff;
	    buf[0] = '\0';
	    qpfPrintf(session, buf+4, 36, "Here is the string: %8STR...", "STRINGSTR");
	    qpfPrintf(session, buf+4, 36, "Here is the string: %8STR...", "STRINGSTR");
	    qpfPrintf(session, buf+4, 36, "Here is the string: %8STR...", "STRINGSTR");
	    rval = qpfPrintf(session, buf+4, 36, "Here is the string: %8STR...", "STRINGSTR");
	    ...
	    /* UTF-8 */
	    rval = qpfPrintf(NULL, buf+4, 36, "ជខ្សែអក្សរ: %8STR....", "ΣEIPA");	    
