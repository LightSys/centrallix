...
httxRender(pHtSession s, pObject w_obj, int z, char* parentname, char* parentobj)
	...
	htrAddScriptFunction(s, "tx_getvalue", "\n"
		"function tx_getvalue(l)\n"
		"    {\n"
		"    var txt = '';\n"
		"    for (var i=0;i<l.rows.length;i++)\n"
		"        {\n"
		"        if (l.rows[i].newLine) txt += '\\\\n'+l.rows[i].content;\n"
		"        else txt += l.rows[i].content;\n"
		"        }\n"
		"    return txt;\n"
		"    }\n", 0);
	...
	htrAddScriptFunction(s, "tx_setvalue", "\n"
		...
		"        tx_wordWrapDown(l,l.rows.length-1,para,0);\n"
		"        if (newL)\n"
		"            {\n"
		"            for(var i=0;i<newL.length;i++)\n"
		"                {\n"
		"                tx_insertRow(l,l.rows.length,'');\n"
		"                l.rows[l.rows.length-1].newLine = 1;\n"
		"                }\n"
		"            }\n"
		"        }\n"
		"    while (txt)\n"
	...
	htrAddScriptFunction(s, "tx_clearvalue", "\n"
		...
		"    {\n"
		"    if (tx_getvalue(l) == '') return;\n"
		"    tx_updateRow(l,0,'');\n"
		"    for(var i=0;i<l.rows.length;i++)\n"
		"        {\n"
		"        l.rows[i].contentLayer.visibility = 'hidden';\n"
		"        l.rows[i].hiddenLayer.visibility = 'hidden';\n"
		"        }\n"
		"    if (l.rows.length > 0) l.rows.splice(1,l.rows.length-1);\n"
		"    if (tx_current == l)\n"
		"        {\n"
		"        l.cursorRow = 0;\n"
		"        l.cursorCol = 0;\n"
