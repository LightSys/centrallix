 ...
 ***/
int
htmapRender(pHtSession s, pWgtrNode map_node, int z)
    {
    char* ptr;
    char name[64];
    char main_bg[128];
	...
	if (wgtrGetPropertyValue(map_node,"x",DATA_T_INTEGER,POD(&x)) != 0) x=0;
	if (wgtrGetPropertyValue(map_node,"y",DATA_T_INTEGER,POD(&y)) != 0) y=0;
	...
	/** Get name **/
	if (wgtrGetPropertyValue(map_node, "name", DATA_T_STRING, POD(&ptr)) != 0) return -1;
