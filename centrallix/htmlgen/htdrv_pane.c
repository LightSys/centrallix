...
htpnRender(pHtSession s, pWgtrNode tree, int z)
	...
	if (style == 2) /* flat */
	    {
	    htrAddStylesheetItem_va(s,"\t#pn%POSmain { POSITION:absolute; VISIBILITY:inherit; overflow:hidden; LEFT:%INTpx; TOP:%INTpx; WIDTH:calc(%POSpx + (100%% - %POSpx) * (1/3)); HEIGHT:calc(%POSpx + (100%% - %POSpx) * (%POS / %POS)); Z-INDEX:%POS;}\n",id,x,y,minW,preW,treePreW,preH,treePreH,flexH,treeFlexH,z);
		if(x > 12) 
		{
			htrAddStylesheetItem_va(s,"\t#pn%POSmain { POSITION:absolute; VISIBILITY:inherit; overflow:hidden; RIGHT:0px; TOP:%INTpx; WIDTH:calc(%POSpx + (100%% - %POSpx) * (2/3)); HEIGHT:calc(%POSpx + (100%% - %POSpx) * (%POS / %POS)); Z-INDEX:%POS;}\n",id,y,preW,treePreW,preH,treePreH,flexH,treeFlexH,z);
		}
	    ...
	    {
	    htrAddStylesheetItem_va(s,"\t#pn%POSmain { POSITION:absolute; VISIBILITY:inherit; overflow: hidden; LEFT:%INTpx; TOP:%INTpx; WIDTH:calc(%POSpx + (100%% - %POSpx) * (1/3)); HEIGHT:calc(%POSpx + (100%% - %POSpx) * (%POS / %POS)); Z-INDEX:%POS;}\n",id,x,y,preW-2*box_offset,treePreW,preH-2*box_offset,treePreH,flexH,treeFlexH,z);
		if(x > 12) 
		{
			htrAddStylesheetItem_va(s,"\t#pn%POSmain { POSITION:absolute; VISIBILITY:inherit; overflow:hidden; RIGHT:0px; TOP:%INTpx; WIDTH:calc(%POSpx + (100%% - %POSpx) * (2/3)); HEIGHT:calc(%POSpx + (100%% - %POSpx) * (%POS / %POS)); Z-INDEX:%POS;}\n",id,y,preW,treePreW,preH,treePreH,flexH,treeFlexH,z);
		}
