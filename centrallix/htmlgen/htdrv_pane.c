...
htpnRender(pHtSession s, pWgtrNode tree, int z)
	...
	if (style == 2) /* flat */
	    {
	    htrAddStylesheetItem_va(s,"\t#pn%POSmain { POSITION:absolute; VISIBILITY:inherit; overflow:hidden; LEFT:%INTpx; TOP:%INTpx; MIN-WIDTH:%POSpx; WIDTH:calc(%POSpx + (100%% - %POSpx) * (%POS / %POS)); MIN-HEIGHT:%POSpx; HEIGHT:calc(%POSpx + (100%% - %POSpx) * (1/3)); Z-INDEX:%POS;}\n",id,x,y,minW,preW,treePreW,minH,preH,treePreH,flexH,treeFlexH,z);
		if(x > 12) 
		{
			htrAddStylesheetItem_va(s,"\t#pn%POSmain { POSITION:absolute; VISIBILITY:inherit; overflow:hidden; RIGHT:0px; TOP:%INTpx; MIN-WIDTH:%POSpx; WIDTH:calc(%POSpx + (100%% - %POSpx) * (%POS / %POS)); MIN-HEIGHT:%POSpx; HEIGHT:calc(%POSpx + (100%% - %POSpx) * (2/3)); Z-INDEX:%POS;}\n",id,y,minW,preW,treePreW,minH,preH,treePreH,flexH,treeFlexH,z);
		}
