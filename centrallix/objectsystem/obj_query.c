...
/*** obj_internal_WildcardMatch() - compare a wildcard string with a constant string
 *** to see if there is a match.  Returns 1 on a match, 0 on no match.
 ***/
int
obj_internal_WildcardMatch(char* pattern, int pattern_len, char* str, int str_len)
    {
    char* astptr;
    char* chptr;
    int leading_len;
	...
	while((chptr = memchr(chptr+1, astptr[1], str_len - (chptr+1 - str))) != NULL)
	    {
	    return obj_internal_WildcardMatch(astptr+1, pattern_len - leading_len - 1, chptr, str_len - (chptr - str));
	    }
...
/*** obj_internal_SetupWildcard_r() - recursive function which does the hard
 ...
 ***/
int
obj_internal_SetupWildcard_r(pXArray pathlist, pObjSession sess, char* orig_path, char* pathbuf, char* last_ending, int n_elements, char* element_list[])
    {
	...
	if (n_elements == 0)
	    ...
	    xaAddItem(pathlist, nmSysStrdup(pathbuf));
	    /*printf("expands to: %s\n", pathbuf);*/
	    }
	else
	    {
	    ...
	    obj = objOpen(sess, pathbuf, OBJ_O_RDONLY, 0600, "system/directory");
	    if (!obj)
		goto finished;
	    info = objInfo(obj);
		...
		if (obj_internal_WildcardMatch(element_list[0]+1, (qptr?qptr:slashptr) - (element_list[0]+1), ptr, strlen(ptr)))
		    {
		    ...
		    if (obj_internal_SetupWildcard_r(pathlist, sess, orig_path, pathbuf, slashptr, n_elements-1, element_list+1) < 0)
			goto error;
		    pathbuf[orig_len] = '\0';
		    }
		objClose(subobj);
...
/*** objWildcardQuery() - take the given pathname with wildcards
 *** in it, and generate a list of objects that match the wildcard pathname.
 ***/
int
objWildcardQuery(pXArray pathlist, pObjSession sess, char* searchpath)
    {
    char* path = nmSysStrdup(searchpath);
	...
	strcpy(pathbuf, "");
	if (obj_internal_SetupWildcard_r(pathlist, sess, path, pathbuf, path, n_elements, element_list) < 0)
	    goto error;
