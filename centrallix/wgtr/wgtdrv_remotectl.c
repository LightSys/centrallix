...
/* 									*/
/* Module: 	wgtr/wgtdrv_remotectl.c						*/
...
/*** wgtrmtVerify - allows the driver to check elsewhere in the tree
 ...
 ***/
int
wgtrmtVerify(pWgtrVerifySession s)
    {
    return 0;
    }


/*** wgtrmtNew - after a node has been filled out with initial values,
...
wgtrmtInitialize()
    {
    char* name = "Remote Control Driver";

	wgtrRegisterDriver(name, wgtrmtVerify, wgtrmtNew);
	wgtrAddType(name, "remotectl");
