...
describe('cxjs_round', () =>
    {
    // Rounding to the nearest integer (default dec). Exact halves round away
    ...
    // Special numeric values pass through; a dec large enough to overflow the
    // 10**dec scale factor to Infinity produces NaN.
    run([
	// Args             Result
	[ [Infinity],       Infinity  ],
	[ [Infinity, 2],    Infinity  ],
    ...
    // Sign of a zero result: strictly positive inputs give +0, while zero and
    ...
    // binary, so they can round down rather than up.
    run([
	// Args            Result
	[ [1.005, 2],      1    ],  // not 1.01
