...
In this way an autoform is a "composite widget", having a simple definition in the .app file but resulting in a potentially complex set of widgets in the actual widget tree to be deployed to the client.  At the time of writing, the composite widget pattern is not used in the codebase.

The main reason that this pattern is not currently used is that it breaks the server-side layouts generated by `apos.c`, causing the widget to always render at the design resolution.  This also breaks the client-side autoscaling, which matches the broken server-side behavior and always renders the widget at its design location.  These issues can likely be overcome by invoking `apos.c` on any new widget trees created, but this pattern has not been tested.
